...
def create_placed_item(item_def: ItemDefinition) -> PlacedItem:
    ...
    return PlacedItem(
        ...
        container_id=item_def.container_id,
        uid=item_def.id
    )
...
async def start_session() -> Dict[str, Any]:
    ...
    return {
        ...
        "item_catalog": {k: v.__dict__ for k, v in ITEM_CATALOG.items()}
    }
    ...
    for i in range(shop_size):
        if random.random() < 0.8:  # 80% chance of item
            ...
            item_info = {
                ...
                "cost": get_shop_cost(item_spec.rarity, tier),
                "min_damage": item_spec.min_damage,
                "max_damage": item_spec.max_damage,
                "cooldown": item_spec.cooldown,
                "cpu_cost": item_spec.cpu_cost,
                "special_effect": item_spec.special_effect
            }
...
async def simulate_battle(request: BattleRequest) -> Dict[str, Any]:
    ...
    if len(request.inventory.items) == 0:
        raise HTTPException(status_code=400, detail="Cannot battle with empty inventory")
    ...
    for item_def in request.inventory.items:
        try:
            ...
            # Apply tier scaling if needed
            placed_item.spec.tier = item_def.tier
    ...
    if battle_result["winner"] == 1:  # Player won
        ...
        gold_reward = gold_reward = 12 if request.round_number <= 3 else 14 if request.round_number <= 6 else 16 if request.round_number <= 9 else 18 if request.round_number <= 12 else 20
...
async def purchase_item(player_id: str, item_id: str) -> Dict[str, Any]:
    ...
    session.current_shop = [
        si if si and si.get("id") != item_id else None 
...
async def get_leaderboard(limit: int = 10) -> List[Dict]:
    ...
    for player_id, session in sessions.items():
        leaderboard.append({
            ...
            "wins": session.wins,
            "losses": session.losses,
            "win_rate": round(session.wins / max(1, session.wins + session.losses) * 100, 1),
