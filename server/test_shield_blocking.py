...
class TestShieldBlocking:
    ...
    def test_shield_blocks_damage(self):
        ...
        attacker = PlacedItem(
            spec=ItemSpec(
                ...
                triggers=[TimerTrigger(
                    cooldown=1.0,
                    cpu_cost=2,
                    effects=[AttackEffect(min_damage=15, max_damage=15, accuracy=1.0)]
                )]
            ),
            position=(0, 0),
            uid="attacker1"
        ...
        block_actions = [a for a in result["actions"] if a.get("a") == ACTION_CODES["BLOCK"]]
        damage_actions = [a for a in result["actions"] if a.get("a") == ACTION_CODES["DAMAGE"]]
    ...
    def test_shield_30_percent_chance(self):
        ...
        for _ in range(10):
            ...
            damage_actions = [a for a in result["actions"] if a.get("a") == ACTION_CODES["DAMAGE"]]
            block_actions = [a for a in result["actions"] if a.get("a") == ACTION_CODES["BLOCK"] and a.get("i") == "shield0"]
        ...
        if total_attacks > 0:
            ...
            assert 0.15 <= block_rate <= 0.45, f"Block rate {block_rate} not close to 30%"
    ...
    def test_shield_cpu_steal(self):
        ...
        cpu_fail_actions = [a for a in result["actions"] if a.get("a") == ACTION_CODES["CPU_FAIL"]]
        ...
        attacker = PlacedItem(
            spec=ItemSpec(
                ...
                triggers=[TimerTrigger(
                    cooldown=2.0,
                    cpu_cost=2,
                    effects=[AttackEffect(min_damage=20, max_damage=20, accuracy=1.0)]
                )]
            ),
            position=(0, 0),
            uid="attacker1"
