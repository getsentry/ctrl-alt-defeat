...
class GridItem:
    ...
    container_id: Optional[str] = None  # ID of container this item is in
...
class Container:
    ...
    shape: ItemShape = field(default_factory=lambda: SHAPES["2x2"])  # Shape it takes in main grid
...
class InventoryGrid:
    ...
    def __init__(self, main_grid_size: Tuple[int, int] = (7, 9)):
        ...
        self.container_positions: Dict[str, Tuple[int, int]] = {}  # Container positions in main grid
    ...
    def can_place_in_main(self, item: GridItem) -> bool:
        ...
        for x, y in occupied:
            if x < 0 or x >= self.main_grid_size[0] or y < 0 or y >= self.main_grid_size[1]:
                return False
    ...
    def render_ascii(self) -> str:
        ...
        # Create display grid
        display = [[" " for _ in range(self.main_grid_size[0])] for _ in range(self.main_grid_size[1])]
        ...
        for container_id, container in self.containers.items():
            ...
            c_display = [[" " for _ in range(container.grid_size[0])] for _ in range(container.grid_size[1])]
            
            for item in container.items.values():
                for x, y in item.get_occupied_squares():
                    if 0 <= x < container.grid_size[0] and 0 <= y < container.grid_size[1]:
...
if __name__ == "__main__":
    ...
    # Test adjacency
    print("\nAdjacency test:")
    print(f"Items adjacent to bug1: {[i.item_id for i in inventory.get_adjacent_items(item1)]}")
    print(f"Items adjacent to shield1: {[i.item_id for i in inventory.get_adjacent_items(item2)]}")