...
class TestWhatAnEffectSays:
    def test_a_weapon_says_what_it_hits_for_and_what_it_costs(self):
        assert line(
            TimerTrigger(cooldown=1.4, cpu_cost=1.0, effects=[
                AttackEffect(min_damage=1, max_damage=3, accuracy=0.85, crit_chance=0.05)])
    ...
    def test_one_number_rather_than_a_range_that_does_not_vary(self):
        assert "deal 5 damage" in line(
            TimerTrigger(cooldown=2, cpu_cost=0, effects=[
                AttackEffect(min_damage=5, max_damage=5, accuracy=0.85, crit_chance=0.05)]))
    ...
    def test_a_status_is_named_as_a_player_reads_it(self):
        ...
        said = line(OnHitTrigger(chance=1.0, effects=[
            DebuffEffect(debuff_name="memory_leaked", value=2)]))
    ...
    def test_the_odds_go_in_front_of_the_whole_clause(self):
        ...
        said = line(OnHitTrigger(chance=1.0, effects=[
            ChanceEffect(chance=0.25, effects=[
                BuffEffect(buff_name="spiked", value=1, target_type="self"),
                AttackEffect(min_damage=2, max_damage=2, accuracy=0.85, crit_chance=0.05)])]))
    ...
    def test_a_run_of_buffs_is_said_once(self):
        ...
        said = line(TimerTrigger(cooldown=8, cpu_cost=0, effects=[
    ...
    def test_the_three_weapon_kinds_are_just_weapons(self):
        said = line(BattleStartTrigger(effects=[
            ModifyEffect(stat="damage", value=0.15, target_type="star", cap=None,
    ...
    def test_tags_an_item_might_carry_are_alternatives(self):
        ...
        said = line(BattleStartTrigger(effects=[
            ModifyEffect(stat="trigger_speed", value=0.1, target_type="star",
                         cap=None, counting={"any": ["pet", "script"]})]))
        assert "pet or script star items" in said

    def test_a_condition_carries_both_halves(self):
        said = line(TimerTrigger(cooldown=1.3, cpu_cost=0, effects=[
            ConditionEffect(
                subject="status", whose="enemy", status="throttled",
                test="at_least", amount=10,
                effects=[BuffEffect(buff_name="monitored", value=1,
                                    target_type="self")],
    ...
    def test_taking_a_status_off_says_who_it_comes_off(self):
        assert "cleanse 2 memory leak from yourself" in line(
            TimerTrigger(cooldown=2, cpu_cost=0, effects=[
                CleanseEffect(count=2, removes="memory_leaked",
                              target_type="self")]))
        assert "remove 1 buff from your opponent" in line(
            TimerTrigger(cooldown=2, cpu_cost=0, effects=[
                CleanseEffect(count=1, removes="buff", target_type="enemy")]))
    ...
    def test_something_always_true_says_only_what_it_does(self):
        ...
        said = line(PassiveTrigger(effects=[
            ModifyEffect(stat="trigger_speed", value=-0.3, target_type="star",
                         cap=None, counting="any")]))
        assert said == "Star items get -30% trigger speed"

    def test_a_status_nobody_chose_has_an_article(self):
        assert "gain a random buff" in line(TimerTrigger(
            cooldown=2, cpu_cost=0, effects=[
                RandomStatusEffect(kind="buff", count=1, target_type="self")]))
        assert "inflict 3 random debuffs" in line(TimerTrigger(
            cooldown=2, cpu_cost=0, effects=[
    ...
    def test_a_number_on_a_player_reads_as_a_sentence(self):
        ...
        said = line(BattleStartTrigger(effects=[
            PlayerModifyEffect(stat="damage_taken", value=-0.25,
                               target_type="self", duration=5.0)]))
        assert said == (
            "Battle start: your damage taken is reduced by 25% for 5s")

    def test_refusing_a_debuff_says_which_way_it_is_refused(self):
        assert "refuse the next 2 debuffs" in line(BattleStartTrigger(effects=[
            ResistEffect(count=2, chance=0, target_type="self")]))
        assert "refuse a debuff 30% of the time" in line(BattleStartTrigger(
            effects=[ResistEffect(count=0, chance=0.3, target_type="self")]))

    def test_statuses_taken_off_the_same_person_say_so_once(self):
        said = line(TimerTrigger(cooldown=4, cpu_cost=0, effects=[
    ...
    def test_a_flat_bonus_is_not_said_as_a_percentage(self):
        ...
        flat = line(PassiveTrigger(effects=[
            ModifyPerStatusEffect(stat="damage_flat", value=1.0,
                                  status="spiked", whose="self")]))
        scaled = line(PassiveTrigger(effects=[
            ModifyPerStatusEffect(stat="damage", value=0.15,
                                  status="spiked", whose="self")]))
...
class TestWhoItLandsOn:
    ...
    def test_cpu_taken_off_the_attacker_comes_off_your_opponent(self):
        said = line(OnAttackedTrigger(chance=0.3, answers_to=frozenset({"melee"}),
                                      effects=[
            CpuDrainEffect(amount=0.3, target_type="attacker")]))
        assert said == (
            "When attacked (Melee, 30%): drain 0.3 CPU from your opponent")

    def test_cpu_taken_off_yourself_is_what_the_item_cost(self):
        said = line(OnAttackedTrigger(chance=0.3, effects=[
            CpuDrainEffect(amount=0.3, target_type="self")]))
    ...
    def test_anything_that_is_not_yourself_is_your_opponent(self):
        ...
        for effect, expected in [
            ...
            (PlayerModifyEffect(stat="healing_taken", value=-0.3,
                                target_type="attacker", duration=-1),
             "your opponent's healing received is reduced by 30%"),
            (DebuffEffect(debuff_name="throttled", value=1,
    ...
    def test_a_buff_is_the_one_that_reads_the_other_way(self):
        ...
        assert plain(describe.of_effect(BuffEffect(
            buff_name="spiked", value=1, target_type="attacker"))) == "gain 1 spiked"
        assert plain(describe.of_effect(BuffEffect(
            buff_name="spiked", value=1, target_type="enemy"))
        ) == "give your opponent 1 spiked"


class TestHowALineIsLaidOut:
    def test_a_trigger_that_does_several_things_lists_them(self):
        assert block(OnAttackedTrigger(
            chance=0.3, answers_to=frozenset({"melee"}), effects=[
    ...
    def test_a_trigger_that_does_one_thing_stays_one_line(self):
        ...
        assert len(block(TimerTrigger(cooldown=2, cpu_cost=0, effects=[
            AttackEffect(min_damage=1, max_damage=2, accuracy=1.0,
                         crit_chance=0.0)]))) == 1
    ...
    def test_standing_clauses_carry_no_bullet(self):
        ...
        said = block(PassiveTrigger(effects=[
            ModifyPerStatusEffect(stat="damage_flat", value=1,
                                  status="spiked", whose="self"),
            ModifyPerStatusEffect(stat="damage_flat", value=1,
                                  status="draining", whose="self")]))
        assert said == [
    ...
    def test_a_cap_over_everything_goes_up_with_the_condition(self):
        said = block(OnHitTrigger(chance=1.0, effects=[
            LimitEffect(times=5, effects=[
                BuffEffect(buff_name="spiked", value=1, target_type="self"),
    ...
    def test_a_condition_that_can_only_come_round_once_says_it_once(self):
        ...
        # twice.
        said = block(describe.HealthThresholdTrigger(threshold=0.7, effects=[
            LimitEffect(times=1, effects=[
                BuffEffect(buff_name="spiked", value=1, target_type="self"),
                describe.BlockEffect(block_amount=15)])]))
...
class TestTheOrderTheyComeIn:
    def test_the_catalogue_order_is_not_the_reading_order(self):
        ...
        spec.triggers = [
            ...
            OnHitTrigger(chance=1.0, effects=[
                BuffEffect(buff_name="spiked", value=1, target_type="self")]),
            TimerTrigger(cooldown=2, cpu_cost=0, effects=[
                describe.BlockEffect(block_amount=5)]),
    ...
    def test_two_of_a_kind_keep_the_order_they_were_written_in(self):
        ...
        spec = deepcopy(ITEM_CATALOG["null_blade"])
        spec.triggers = [
            TimerTrigger(cooldown=4, cpu_cost=0, effects=[
                describe.BlockEffect(block_amount=3)]),
            TimerTrigger(cooldown=4, cpu_cost=0, effects=[
                describe.BlockEffect(block_amount=9)]),
        ]
    ...
    def test_every_kind_of_trigger_has_a_place_in_the_order(self):
        ...
        kinds = {
            kind for kind in vars(item_effects).values()
...
class TestTheWholeCatalogue:
    ...
    def test_every_status_is_marked_as_the_kind_of_status_it_is(self):
        ...
        said = describe.block(OnHitTrigger(chance=1.0, effects=[
            BuffEffect(buff_name="spiked", value=1, target_type="self"),
            DebuffEffect(debuff_name="throttled", value=2)]))

        assert said[1] == "• Gain 1 [buff]spiked[/buff]"
        assert said[2] == (
            "• Apply 2 [debuff]throttled[/debuff] to your opponent")
