...
class TestDeterministicBattles:
    ...
    def test_same_seed_same_result(self):
        ...
        attacker = PlacedItem(
            spec=ItemSpec(
                ...
                triggers=[TimerTrigger(
                    cooldown=1.0,
                    cpu_cost=2,
                    effects=[AttackEffect(min_damage=3, max_damage=10, accuracy=0.8, crit_chance=0.2)]
                )]
            ),
            position=(0, 0),
            uid="attacker1"
        ...
        defender = PlacedItem(
            spec=ItemSpec(
                ...
                triggers=[TimerTrigger(
                    cooldown=1.5,
                    cpu_cost=3,
                    effects=[AttackEffect(min_damage=4, max_damage=8, accuracy=0.9, crit_chance=0.1)]
                )]
            ),
            position=(0, 0),
            uid="defender1"
        ...
        result1 = sim1.simulate_battle([deepcopy(attacker)], [deepcopy(defender)], round_number=1)
        
        # Run again with same seed
        sim2 = BattleSimulator(seed=12345)
        result2 = sim2.simulate_battle([deepcopy(attacker)], [deepcopy(defender)], round_number=1)
        ...
        for i, (action1, action2) in enumerate(zip(result1["actions"], result2["actions"])):
    ...
    def test_different_seed_different_result(self):
        ...
        rng_item = PlacedItem(
            spec=ItemSpec(
                ...
                triggers=[TimerTrigger(
                    cooldown=0.5,
                    cpu_cost=1,
                    effects=[AttackEffect(min_damage=1, max_damage=20, accuracy=0.5, crit_chance=0.5)]
                )]
            ),
            position=(0, 0),
            uid="rng1"
        ...
        for seed in [100, 200, 300, 400, 500]:
            ...
            result = sim.simulate_battle([deepcopy(rng_item)], [deepcopy(rng_item)], round_number=1)
        ...
        assert len(unique_winners) > 1 or len(unique_quotas) > 2, \
    ...
    def test_shield_blocking_deterministic(self):
        ...
        attacker = PlacedItem(
            spec=ItemSpec(
                ...
                triggers=[TimerTrigger(
                    cooldown=0.3,
                    cpu_cost=1,
                    effects=[AttackEffect(min_damage=3, max_damage=3, accuracy=1.0)]
                )]
            ),
            position=(0, 0),
            uid="attacker1"
        ...
        for _ in range(3):
            sim = BattleSimulator(seed=seed)
            result = sim.simulate_battle([deepcopy(shield)], [deepcopy(attacker)], round_number=1)
            results.append(result)
        ...
        for i in range(1, len(results)):
            ...
            blocks_0 = len([a for a in results[0]["actions"] if a.get("a") == ACTION_CODES["BLOCK"]])
            blocks_i = len([a for a in results[i]["actions"] if a.get("a") == ACTION_CODES["BLOCK"]])
            assert blocks_0 == blocks_i, f"Block count differs: {blocks_0} != {blocks_i}"
    ...
    def test_complex_battle_deterministic(self):
        ...
        p1_items = [
            PlacedItem(
                spec=ItemSpec(
                    ...
                    triggers=[TimerTrigger(
                        cooldown=1.0,
                        cpu_cost=2,
                        effects=[AttackEffect(min_damage=2, max_damage=8, accuracy=0.7, crit_chance=0.15)]
                    )]
                ),
                position=(0, 0),
                uid="p1_1"
            ...
            PlacedItem(
                spec=ItemSpec(
                    ...
                    category="defense",
                    triggers=[OnAttackedTrigger(
                        effects=[ShieldBlockEffect(block_chance=0.3, block_amount=4)]
                    )]
                ),
                position=(1, 0),
                uid="p1_2"
            ...
            PlacedItem(
                spec=ItemSpec(
                    ...
                    triggers=[TimerTrigger(
                        cooldown=0.8,
                        cpu_cost=3,
                        effects=[AttackEffect(min_damage=3, max_damage=7, accuracy=0.85, crit_chance=0.1)]
                    )]
                ),
                position=(0, 0),
                uid="p2_1"
            ...
            PlacedItem(
                spec=ItemSpec(
                    ...
                    triggers=[TimerTrigger(
                        cooldown=1.2,
                        cpu_cost=2,
                        effects=[AttackEffect(min_damage=4, max_damage=6, accuracy=0.95, crit_chance=0.05)]
                    )]
                ),
                position=(1, 0),
                uid="p2_2"
        ...
        result1 = sim1.simulate_battle(deepcopy(p1_items), deepcopy(p2_items), round_number=1)
        
        sim2 = BattleSimulator(seed=seed)
        result2 = sim2.simulate_battle(deepcopy(p1_items), deepcopy(p2_items), round_number=1)
