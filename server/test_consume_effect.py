...
from item_effects import (
    ItemSpec, DamageTakenTrigger, HealEffect, ConsumeEffect, 
    BattleStartTrigger, StatModEffect, TimerTrigger, AttackEffect
)
...
class TestConsumeEffect:
    ...
    def test_health_potion_consumes_on_use(self):
        ...
        from item_effects import TimerTrigger, AttackEffect
        attacker = PlacedItem(
            spec=ItemSpec(
                id="attacker",
                name="Attacker",
        ...
        potion1 = PlacedItem(
            spec=ItemSpec(
                ...
                triggers=[DamageTakenTrigger(
                    ...
                    cpu_cost=0,
                    effects=[
                        HealEffect(min_heal=5, max_heal=5),
                        ConsumeEffect()
                    ]
                )]
            ),
        ...
        attacker = PlacedItem(
            spec=ItemSpec(
                ...
                triggers=[TimerTrigger(
                    cooldown=0.5,
                    cpu_cost=2,
                    effects=[AttackEffect(min_damage=12, max_damage=12, accuracy=1.0)]
                )]
            ),
            position=(0, 0),
            uid="attacker1"
